...
class HeatmapLoss(nn.Module):
        ...
        def forward(self, pred, gt, mask, delta):
            assert pred.size() == gt.size()
           
            mask_center = (gt ==1.).float()
            mask_bg = (gt ==0.).float()
            mask_adj = 1. - (mask_center + mask_bg)
            factor = mask_center*torch.abs(1-pred) + mask_bg*torch.abs(pred) + mask_adj* torch.abs((gt ** delta) - pred)*(gt ** delta)
            weight = factor + (1-factor)* torch.abs(gt-pred)

            loss = ((pred - gt) ** 2) * weight * mask[:, None, :, :].expand_as(pred)
